...
gulp.task("environment.chalice_s3_store_state", function() {
  ...
  var promise = gulp
    ...
    .pipe(
      data(function(file) {
        ...
        file.data = JSON.parse(
          fs.readFileSync(config.files.environment_settings)
        );
        return file.data;
      })
    )
    ...
    .pipe(
      data(function(file) {
        console.log(
          "Uploading " + file.data.key + " to " + file.data.bucket_name
        );
...
gulp.task("environment.chalice_s3_retrieve_state", function() {
  ...
  var promise = gulp
    ...
    .pipe(
      data(function(file) {
        console.log(
          "Downloading " + file.data.key + " from " + file.data.bucket_name
        );
...
gulp.task("environment.chalice_s3_delete_state", function() {
  ...
  var promise = gulp
    ...
    .pipe(
      data(function(file) {
        console.log(
          "Deleting " + file.data.key + " from " + file.data.bucket_name
        );
...
gulp.task("environment.chalice_recover_state", function() {
  ...
  var config = helpers.getConfigLocations(env, tool);
  var archive_state =
    config.paths.chalice_deployed + "/" + state + "/" + env + ".json";
...
gulp.task("environment.chalice_config", function() {
  ...
  var pipeline = gulp
    .src(config.files.default_chalice_config)
    .pipe(
      data(function(file) {
        // read env settings file into file.data
        file.data = JSON.parse(
    ...
    .pipe(
      modifyFile(function(content, path, file) {
        ...
        file.data.config.stages[env].environment_variables.CSW_ENV =
          file.data.environment;
        file.data.config.stages[env].environment_variables.CSW_PREFIX =
          file.data.prefix;
        file.data.config.stages[env].environment_variables.CSW_PASSWORD =
          file.data.postgres_user_password;
        file.data.config.stages[env].environment_variables.CSW_HOST =
        ...
        var role_arn =
          "arn:aws:iam::" + file.data.host_account_id + ":role/" + role_name;
        file.data.config.stages[env].iam_role_arn = role_arn;
        var subnets = [
          file.data.public_subnet_1_id,
          file.data.public_subnet_2_id
        ];
        var security_groups = [file.data.public_security_group_id];
        ...
        for (lambda in file.data.config.stages[env].lambda_functions) {
          console.log(lambda);
          file.data.config.stages[env].lambda_functions[
            lambda
          ].subnet_ids = subnets;
          file.data.config.stages[env].lambda_functions[
            lambda
...
gulp.task("environment.chalice_deploy", function() {
  ...
  var pipeline = gulp
    .src(config.paths.chalice_environment)
    ...
    .pipe(
      data(function(file) {
        ...
        return helpers.runTaskInPipelinePromise(
          task,
          config.paths.chalice_code,
          file
        );
      })
    );
