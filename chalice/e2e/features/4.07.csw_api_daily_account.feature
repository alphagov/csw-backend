# Cloud Security Watch - API /daily/account
# Check that a status=ok response is returned and that the fields exists and have the correct types.
Feature: Cloud Security Watch - API /daily/account
    Scenario: can load api/daily/account endpoint
        When you make an unauthenticated http get request to "api/daily/account"
        Then response code is "403"
        Then body is valid json
        Then "status" exists and has value "failed"
        Then "message" exists and has value "Unauthorised"
        When you make an authenticated http get request to "api/daily/account"
        ...
        Then "items.0.account_id" exists and has datatype "dict"
            ...
            Then "items.0.account_id.product_team_id" exists and has datatype "dict"
