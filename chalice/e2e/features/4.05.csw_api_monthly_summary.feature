# Cloud Security Watch - API /monthly/summary
# Check that a status=ok response is returned and that the fields exists and have the correct types.
Feature: Cloud Security Watch - API /monthly/summary
    Scenario: can load api/monthly/summary endpoint
        When you make an unauthenticated http get request to "api/monthly/summary"
        Then response code is "403"
        Then body is valid json
        Then "status" exists and has value "failed"
        Then "message" exists and has value "Unauthorised"
        When you make an authenticated http get request to "api/monthly/summary"
