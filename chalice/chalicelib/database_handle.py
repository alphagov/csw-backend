...
class DatabaseHandle:
    ...
    def get_handle(self):
        if not self.handle or self.handle.is_closed():
            ...
            self.handle = postgres_ext.PostgresqlExtDatabase(
                "csw", user=db_user, password=db_password, host=db_host, port=db_port
            )
        return self.handle
    ...
    def get_models(self):
        ...
        return {
            ...
            for name, cls in inspect.getmembers(
                importlib.import_module("chalicelib.models")
            )
    ...
    def create_or_update_criterion(self, event):
        ...
        db = self.get_handle()
        db.connect()
        recs = Criterion.select().where(
            Criterion.invoke_class_name == event["criterion_name"]
        )
        count = recs.count()
        obj = getattr(
            ...
            self.app
        )  # instantiate the appropriate Criterion subclass
        if count < 1 and obj.active is True:  # new and active criteria
            ...
            existing.invoke_class_get_data_method = (
                "doesn't matter, we should get rid of this column"
            )
