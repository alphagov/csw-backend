...
class AwsIamValidateInspectorPolicy(CriteriaDefault):
    ...
    title = (
        "IAM Roles: The inspector role policy for Cloud Security Watch is up to date"
    )
    ...
    def get_data(self, session, **kwargs):
        ...
        self.app.log.debug(
            "Policies equal = " + str(applied_policy_json == standard_policy_json)
        )
    ...
    def translate(self, data):
        ...
        item = {"resource_id": data["RoleName"], "resource_name": data["RoleName"]}
    ...
    def get_current_policy_version(self, session):
        ...
        try:
            ...
            current_policy_account = current_policy_default.replace(
                "${account_id}", account
            )
            current_policy_account = current_policy_account.replace(
