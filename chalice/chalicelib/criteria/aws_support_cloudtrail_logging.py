...
class CloudtrailLogHasErrors(CloudtrailLogging):
    ...
    def __init__(self, app):
        ...
        self.description = (
            "Checks that CloudTrail is able to write successfully to the S3 bucket."
        )
...
class CloudtrailLogNotInRegion(CloudtrailLogging):
    ...
    def __init__(self, app):
        ...
        self.how_do_i_fix_it = "Make sure you enable CloudTrail in every region."
    ...
    def evaluate(self, event, item, whitelist=[]):
        ...
        if (
            item["metadata"][2] != "On"
        ):  # alternatively =='Off' if you don't want it triggered when trail disabled
            compliance_type = "NON_COMPLIANT"
...
class CloudtrailLogTurnedOff(CloudtrailLogging):
    ...
    def __init__(self, app):
        ...
        self.how_do_i_fix_it = "Make sure that logging is switched on for all regions."
        super(CloudtrailLogTurnedOff, self).__init__(app)

    def evaluate(self, event, item, whitelist=[]):
        compliance_type = "COMPLIANT"
        if (
            item["metadata"][1] is None
            or item["metadata"][2] == "Not enabled"
            or item["metadata"][3] is None
        ):
            compliance_type = "NON_COMPLIANT"
...
class CloudtrailLogNotToCST(CloudtrailLogging):
    ...
    def evaluate(self, event, item, whitelist=[]):
        ...
        if item["metadata"][3] != self.cst_bucket_name:
            compliance_type = "NON_COMPLIANT"
            self.annotation = f'Trail {item["metadata"][1]} in region {item["metadata"][0]} is not sent to {self.cst_bucket_name}'
        return self.build_evaluation(
            item["resourceId"],
            compliance_type,
            event,
