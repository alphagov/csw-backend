...
class AwsIamRolesWithTrustRelationship(CriteriaDefault):
    ...
    why_is_it_important = (
        "Delivery accounts are set up such that there need not be any IAM users in them, "
    ...
    how_do_i_fix_it = (
        "Create a role that trusts an IAM user from a separate account, to allow them to assume a "
    ...
    def retrieve_user_account(self):
        return (
            "622626885786"
        )  # TODO: Find a better way to retrieve the trusted account number
    ...
    def evaluate(self, event, role, whitelist=[]):
        ...
        self.app.log.debug(
            f"Evaluating role with name {role['RoleName']} and ARN {role['Arn']}"
        )
        ...
        if "AWS" in principal:
            ...
            if set(users) == {None}:  # No matches
                ...
                self.annotation = (
                    "<p>No trusted users: This role does not define any IAM users.</p>"
                ...
                for user_match in users:
                    if user_match and user_match.group(1) != self.user_account:
                        compliance_type = "NON_COMPLIANT"
                        self.app.log.debug(
                            "Role has an IAM User from unknown AWS account defined in its trust policy"
                        )
                        self.annotation = (
                            "<p>Untrusted user: This role trusts an IAM user from an AWS account that "
                    ...
                    self.annotation = ""
                    self.app.log.debug(
                        f"Role: {role['RoleName']} is found to be compliant"
                    )
            ...
            self.annotation = (
                "<p>Invalid service: This role trusts a service, such as EC2 or Lambda, "
