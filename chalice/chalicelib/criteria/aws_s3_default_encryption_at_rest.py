...
class AwsS3DefaultEncryptionAtRest(CriteriaDefault):
    ...
    description = (
        "The S3 bucket does have encryption enabled at rest to automatically encrypt all objects when "
        "stored in Amazon S3"
    )
    why_is_it_important = (
        "S3 buckets with default encryption will enable Amazon to encrypt your S3 data at the "
        "bucket level instead of object level in order to protect it from attackers or unauthorized "
        "personnel."
    )
    how_do_i_fix_it = (
        "Enable encryption during the upload process using Server-Side Encryption with either AWS "
    ...
    def get_data(self, session, **kwargs):
        ...
        for bucket in buckets:
            bucket["Encryption"] = self.client.get_bucket_encryption(
                session, bucket["Name"]
            )
        return buckets
