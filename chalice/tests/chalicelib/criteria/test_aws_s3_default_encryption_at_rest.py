from chalicelib.criteria.aws_s3_default_encryption_at_rest import (
    AwsS3DefaultEncryptionAtRest,
)
...
class TestAwsS3DefaultEncryptionAtRest(
    ...
    def test_get_data(self):
        for key in self.test_data:
            with self.subTest(key=key):
                ...
                self.subclass.client.get_bucket_encryption = lambda session, name: self.test_data[
                    key
                ][
                    0
                ][
                ...
                self.assertIn(
                    "Encryption",
                    item[0],
                    msg="The dictionary must have an encryption key",
                )
