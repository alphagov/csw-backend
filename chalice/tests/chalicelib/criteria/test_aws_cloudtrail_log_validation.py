...
class TestValidationCloudtrail(CriteriaSubclassTestCaseMixin, TestCaseWithAttrAssert):

    @classmethod
    def setUpClass(cls):
        super(TestValidationCloudtrail, cls).setUpClass()
        cls.test_data = CLOUDTRAIL_MULTIREGIONAL

    def setUp(self):
        """
        """
        super(TestValidationCloudtrail, self).setUp()
        self.subclass = CloudTrailFileValidation(self.app)
