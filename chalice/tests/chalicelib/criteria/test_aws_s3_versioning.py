    ...
    def test_get_data(self):
        for key in self.test_data:
            with self.subTest(key=key):
                ...
                self.subclass.client.get_bucket_versioning = lambda session, bucket_name: self.test_data_status[
                    key
                ]
                ...
                for bucket in item:
                    self.assertIn(
                        "Versioning",
                        bucket,
                        msg="The dicts within the list must have a 'Versioning' key",
                    )
                    self.assertIsInstance(
                        bucket["Versioning"], dict, msg="Versioning must be a dict"
                    )
